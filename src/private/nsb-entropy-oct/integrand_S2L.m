function f = integrand_S2L (xi)
  ...
  %% wrt the average <S>, with the left part of  the evidence copied.
  %% The value so obtained should be used for calculating the left error
  %% bar.
  %%
  %% In:
  ...
  xi_mirror = xi;
  gS_nsb    = find(xi>S_nsb);
  xi_mirror (gS_nsb) = 2*S_nsb - xi_mirror(gS_nsb);

  B=B_xiK(xi, nsb_K_quad);
  B_mirror=B_xiK(xi_mirror,nsb_K_quad);

  f = exp(-mlog_evidence(B_mirror,nsb_kx_quad,nsb_nx_quad,nsb_K_quad) + ...
	  nsb_mlog_quad).*prior_xi(xi_mirror,nsb_K_quad).*meanS2(B,nsb_kx_quad,nsb_nx_quad,nsb_K_quad);
