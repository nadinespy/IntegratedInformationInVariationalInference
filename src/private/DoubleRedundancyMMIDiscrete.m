...
%   Y1, Y2).
%
%   R = DOUBLEREDUNDANCYMMIDISCRETE(..., E) uses estimator E to estimate mutual
%   information. Implemented estimators are 'PlugIn' and 'NSB' (default: 'NSB')
%
...
elseif nargin == 3
  R = private_TDMMI(varargin{1}, varargin{2}, varargin{3});
elseif nargin == 4
  R = private_FourVectorMMI(varargin{1}, varargin{2}, varargin{3}, varargin{4});
elseif nargin == 5
  R = private_FourVectorMMI(varargin{1}, varargin{2}, varargin{3}, varargin{4}, varargin{5});
else
...
%*********************************************************
%*********************************************************
function [ redred ] = private_TDMMI(X, tau, estimator)
...
redred = private_FourVectorMMI(X(p1, 1:end-tau), X(p2, 1:end-tau), ...
                                X(p1, 1+tau:end), X(p2, 1+tau:end), estimator);
...
%*********************************************************
%*********************************************************
function [ redred ] = private_FourVectorMMI(X1, X2, Y1, Y2, estimator)
